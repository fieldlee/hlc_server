...
func (this Remote)AssetButcher(args map[string]map[string][]string, result *Asset) error {
	...
	_, ok = mx["operation"]
	if !ok {
		result.Message = "operation required"
		return nil
	}
...
func (this Remote)AssetWaitButcher(args map[string]map[string][]string, result *Asset) error {
	...
	_, ok = mx["waitButcherTime"]
	if !ok {
		result.Message = "waitButcherTime required"
		return nil
	}
	switch mx["waitButcherTime"].(type) {
	case string:
	default:
		result.Message = "waitButcherTime should be string"
		return nil
	}
